...
module.exports = (client) => {
  ...
  app.get("/dashboard/:guildID/msgleaderboard", checkAuth, async (req, res) => {
    let idguild = JSON.parse(fs.readFileSync("./datajsons/GUILDID.json", "utf8"));
    if(!idguild){
      return idguild = {
   ...
   idguild = {
    id: req.params.guildID
     ...
     fs.writeFile("./datajsons/GUILDID.json", JSON.stringify(idguild), (err) => {
       if (err) console.log(err);
     });
     const SQLite = require("better-sqlite3");
    const sql = new SQLite('./SQL/msgleaderboard/msg.sqlite');
    const points = sql.prepare("SELECT * FROM scores WHERE guild = ? ORDER BY points DESC LIMIT 10;").all(idguild.id);
    const guild = client.guilds.get(idguild.id);
    if (!guild) return res.status(404);
